...
```
**LP has higher totalPopulation, lower poverty and higher Employment than UP.
...
ggplot(county_MI) + geom_point(aes(x=county_MI$Unemployment, y=county_MI$TotalPop)) + facet_wrap(~peninsula)
```
...
```
Because total population value is too large and y axis is limited, then most of the point would be gathered together in the bottom, using log10 can make the number smaller and easier to differentiate from each other, which would make the plot looks nicer. 
